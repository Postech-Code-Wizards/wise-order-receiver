...
@Component
@Slf4j
public class TicketValidationStrategy implements PaymentValidationStrategy {

    @Override
    public void validate(PaymentMethod paymentMethod) {
        log.info("Starting validation for Ticket.");
        if (StringUtils.hasText(paymentMethod.getCardNumber())
            ...
            throw new BusinessException(HttpStatus.BAD_REQUEST.name()
                    ...
                    , "Ticket payment method should not contain card details.");
        }
        log.info("Ticket validation completed successfully.");
    }

    @Override
    public PaymentMethodTypeEnum getType() {
        return PaymentMethodTypeEnum.TICKET;
    }
}
